 ...
 * Class for HTTP request transport using a proxy server
 *
 * @class Transport~CordovaTransport
 * @protected
 * @extends Transport
 */
var CordovaTransport = Transport.CordovaTransport = function() {};

inherits(CordovaTransport, Transport);
 ...
 * @returns {Promise.<Object>}
 */
CordovaTransport.prototype.httpRequest = function(params, callback) {
  var deferred = Promise.defer();
  var req;
  var httpRequest = this._getHttpRequestModule();
  var sent = false;
  var createRequest = function() {
    if (!sent) {
      ...
      if (params.headers["Content-Type"] == "text/xml") {
        httpRequest.setDataSerializer( "utf8" );
      }
      else {
        httpRequest.setDataSerializer( "json" );
        if (params.body) {
          params.data = JSON.parse(params.body);
        }
      }
      
      req = httpRequest.sendRequest(url, params, function(response) {
...
CordovaTransport.prototype._getHttpRequestModule = function() {
