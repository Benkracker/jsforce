...
var Connection = module.exports = function(options) {
  ...
  if (window.hasOwnProperty('cordova')) {
    this._transport = new Transport.CordovaTransport();
 ...
 * @returns {Promise.<UserInfo>}
 */
Connection.prototype.login = function(username, password, orgId, callback) {
  ...
  if (this.oauth2 && this.oauth2.clientId && this.oauth2.clientSecret) {
    ...
    return this.loginBySoap(username, password, orgId, callback);
  }
};
 ...
 * @returns {Promise.<UserInfo>}
 */
Connection.prototype.loginBySoap = function(username, password, orgId, callback) {
  var self = this;
  var logger = this._logger;
  var body = [
    '<se:Envelope xmlns:se="http://schemas.xmlsoap.org/soap/envelope/" xmlns:urn="urn:partner.soap.sforce.com">',
      '<se:Header>',
      '<urn:LoginScopeHeader>',
        '<urn:organizationId>' + esc(orgId) + '</urn:organizationId>',
      '</urn:LoginScopeHeader>',
      '</se:Header>',
      '<se:Body>',
